...
from pointconv_util import PointConvD, PointWarping, UpsampleFlow, CrossLayerLight as CrossLayer
...
from pointconv_util import SA_Layer, Flow_Fine_Offset
import time

scale = 1.0


class PointConvSFE(nn.Module):
    def __init__(self):
        super(PointConvSFE, self).__init__()
        ...
        self.sa_layer2 = SA_Layer(128, 8)
        self.flow2 = SceneFlowEstimatorResidual(128 + 64, 128)
        self.flow2_fine = Flow_Fine_Offset(64)
        ...
        self.sa_layer3 = SA_Layer(256, 8)
        self.flow3 = SceneFlowEstimatorResidual(256, 256)
        self.flow3_fine = Flow_Fine_Offset(64)
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        flow3_offset = self.flow3_fine(feat3)
        fine_flow3 = flow3 + flow3_offset
        ...
        flow2_offset = self.flow2_fine(feat2)
        fine_flow2 = flow2 + flow2_offset
        ...
        flow1_offset = self.flow1_fine(feat1)
        fine_flow1 = flow1 + flow1_offset
        ...
        flow0_offset = self.flow0_fine(feat0)
        fine_flow0 = flow0 + flow0_offset

        flows = [flow0, flow1, flow2, flow3]
        flows_fine = [fine_flow0, fine_flow1, fine_flow2, fine_flow3]
        ...
        return flows, flows_fine, fps_pc1_idxs, fps_pc2_idxs, pc1, pc2


def multiScaleLoss(pred_flows, pred_flows_fine, gt_flow, fps_idxs, alpha=[0.02, 0.04, 0.08, 0.16]):
    # num of scale
    ...
    for i in range(num_scale):
        ...
        diff_flow_fine = pred_flows_fine[i].permute(0, 2, 1) - gt_flows[i + offset]
        total_loss += alpha[i] * (0.5 * torch.norm(diff_flow, dim=2).sum(dim=1).mean() +
                                  torch.norm(diff_flow_fine, dim=2).sum(dim=1).mean())
    return total_loss
